    ...
    <div class="container">
        <div class="row">
            <div class="col-md-7">
                ...
                <form action="{{ route('admins.categories.update', $category->slug) }}" method="POST"
                    ...
                    <div class="form-floating mb-3">
                        <input id="name" class="form-control @error('name') is-invalid @enderror" name="name"
                            placeholder="{{ __('name') }}" value="{{ old('name', $category->name) }}" autofocus required>
                        <label for="name">
                            {{ __('name') }}
                        </label>
                    ...
                    <div class="form-floating mb-3">
                        <input id="slug" class="form-control @error('slug') is-invalid @enderror" name="slug"
                            placeholder="{{ __('slug') }}" value="{{ old('slug', $category->slug) }}" required>
                        <label for="slug">
                            {{ __('slug') }}
                        </label>
                    ...
                    <div class="form-floating mb-3">
                        <select id="parentId" name="parent_id" class="form-select @error('parent_id') is-invalid @enderror"
                            ...
                            @foreach ($categories as $categ)
                                <option value="{{ $categ->id }}" @selected(old('parent_id', $category->parent_id) == $categ->id)>
                                    {{ $categ->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="p-2 mb-3 text-center border rounded">
                        <img class="img-fluid rounded" src="{{ Storage::url($category->getFeaturedImage()) }}"
                            alt="{{ __($category->slug . ' ' . 'featured_image') }}" style="height:150px">
                    </div>
